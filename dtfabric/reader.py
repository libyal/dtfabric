...
class DataTypeDefinitionsReader:
    ...
    _INTEGER_FORMAT_ATTRIBUTES = frozenset(
        [definitions.FORMAT_SIGNED, definitions.FORMAT_UNSIGNED]
    )
    ...
    _SUPPORTED_DEFINITION_VALUES_LAYOUT_ELEMENT = set(["data_type", "offset"])
    ...
    _SUPPORTED_DEFINITION_VALUES_STORAGE_DATA_TYPE = set(["attributes"]).union(
        _SUPPORTED_DEFINITION_VALUES_DATA_TYPE
    )

    _SUPPORTED_DEFINITION_VALUES_STORAGE_DATA_TYPE_WITH_MEMBERS = set(
        ["members"]
    ).union(_SUPPORTED_DEFINITION_VALUES_STORAGE_DATA_TYPE)

    _SUPPORTED_DEFINITION_VALUES_CONSTANT = set(["value"]).union(
        _SUPPORTED_DEFINITION_VALUES_DATA_TYPE
    )

    _SUPPORTED_DEFINITION_VALUES_ENUMERATION = set(["values"]).union(
        _SUPPORTED_DEFINITION_VALUES_DATA_TYPE
    )
    ...
    ).union(_SUPPORTED_DEFINITION_VALUES_DATA_TYPE)
    ...
    ).union(_SUPPORTED_DEFINITION_VALUES_MEMBER_DATA_TYPE)

    _SUPPORTED_DEFINITION_VALUES_FORMAT = set(
        ["attributes", "layout", "metadata"]
    ).union(_SUPPORTED_DEFINITION_VALUES_DATA_TYPE)

    _SUPPORTED_DEFINITION_VALUES_PADDING = set(["alignment_size"]).union(
        _SUPPORTED_DEFINITION_VALUES_MEMBER_DATA_TYPE
    )

    _SUPPORTED_DEFINITION_VALUES_STRING = set(["encoding"]).union(
        _SUPPORTED_DEFINITION_VALUES_ELEMENTS_DATA_TYPE
    )

    _SUPPORTED_DEFINITION_VALUES_STRING_MEMBER = set(["encoding"]).union(
        _SUPPORTED_DEFINITION_VALUES_ELEMENTS_MEMBER_DATA_TYPE
    )

    _SUPPORTED_DEFINITION_VALUES_STRUCTURE_FAMILY = set(["base", "members"]).union(
        _SUPPORTED_DEFINITION_VALUES_DATA_TYPE
    )

    _SUPPORTED_DEFINITION_VALUES_STRUCTURE_GROUP = set(
        ["base", "default", "identifier", "members"]
    ).union(_SUPPORTED_DEFINITION_VALUES_DATA_TYPE)

    _SUPPORTED_ATTRIBUTES_STORAGE_DATA_TYPE = set(["byte_order"])

    _SUPPORTED_ATTRIBUTES_FIXED_SIZE_DATA_TYPE = set(["size", "units"]).union(
        _SUPPORTED_ATTRIBUTES_STORAGE_DATA_TYPE
    )

    _SUPPORTED_ATTRIBUTES_BOOLEAN = set(["false_value", "true_value"]).union(
        _SUPPORTED_ATTRIBUTES_FIXED_SIZE_DATA_TYPE
    )

    _SUPPORTED_ATTRIBUTES_FORMAT = set(["byte_order"])

    _SUPPORTED_ATTRIBUTES_INTEGER = set(["format"]).union(
        _SUPPORTED_ATTRIBUTES_FIXED_SIZE_DATA_TYPE
    )

    def _ReadBooleanDataTypeDefinition(
        self, definitions_registry, definition_values, definition_name, is_member=False
    ):
    ...
    def _ReadConstantDataTypeDefinition(
        ...
        """
        if is_member:
            raise errors.DefinitionReaderError(
                definition_name, ("data type not supported as member")
            )

        value = definition_values.get("value")
        if value is None:
        ...
        definition_object = self._ReadSemanticDataTypeDefinition(
            ...
            definition_name,
            self._SUPPORTED_DEFINITION_VALUES_CONSTANT,
        )
    ...
    def _ReadDataTypeDefinition(
        ...
        if unsupported_definition_values:
            ...
            raise errors.DefinitionReaderError(
                definition_name, (f"unsupported definition values: {values_string:s}")
            )

        aliases = definition_values.get("aliases")
        description = definition_values.get("description")
        urls = definition_values.get("urls")
    ...
    def _ReadDataTypeDefinitionWithMembers(
        ...
        Raises:
          DefinitionReaderError: if the definitions values are missing or if
              the format is incorrect.
        """
        members = definition_values.get("members")
        if not members:
        ...
        if attributes:
            ...
            if unsupported_attributes:
                ...
                raise errors.DefinitionReaderError(
                    definition_name, (f"unsupported attributes: {attributes_string:s}")
                )
    ...
    def _ReadEnumerationDataTypeDefinition(
        ...
        for enumeration_value in values:
            aliases = enumeration_value.get("aliases")
            description = enumeration_value.get("description")
            name = enumeration_value.get("name")
            number = enumeration_value.get("number")
            ...
            if not name or number is None:
                ...
                raise errors.DefinitionReaderError(
                    definition_name, (f"{error_location:s} missing name or number")
                )
    ...
    def _ReadElementSequenceDataTypeDefinition(
        ...
        element_data_type = definition_values.get("element_data_type")
        if not element_data_type:
            raise errors.DefinitionReaderError(
                definition_name, ("missing element data type")
            )

        elements_data_size = definition_values.get("elements_data_size")
        elements_terminator = definition_values.get("elements_terminator")
        number_of_elements = definition_values.get("number_of_elements")
        ...
        if not size_values:
            message = (
                "missing element data size, elements terminator and number of elements"
            )
            raise errors.DefinitionReaderError(
                definition_name,
                message,
        ...
        if not element_data_type_definition:
            message = f"undefined element data type: {element_data_type:s}"
            raise errors.DefinitionReaderError(
                definition_name,
                message,
            )
    ...
    def _ReadFixedSizeDataTypeDefinition(
        ...
        default_size=definitions.SIZE_NATIVE,
        default_units="bytes",
        is_member=False,
    ...
    def _ReadFormatDataTypeDefinition(
        ...
        definition_object = self._ReadLayoutDataTypeDefinition(
            ...
            definition_name,
            self._SUPPORTED_DEFINITION_VALUES_FORMAT,
        )
    ...
    def _ReadFormatLayout(
        ...
        for index, layout_element in enumerate(definition_values):
            data_type = layout_element.get("data_type")
            offset = layout_element.get("offset")
    ...
    def _ReadMemberDataTypeDefinitionMember(
        ...
        if not definition_values:
            raise errors.DefinitionReaderError(
                definition_name, ("invalid structure member missing definition values")
            )

        name = definition_values.get("name")
        type_indicator = definition_values.get("type")

        if not name and type_indicator != definitions.TYPE_INDICATOR_UNION:
            raise errors.DefinitionReaderError(
                definition_name, ("invalid structure member missing name")
            )

        # TODO: detect duplicate names.

        data_type = definition_values.get("data_type")
        ...
        condition = definition_values.get("condition")
        ...
        if type_indicator is not None:
            ...
            if not data_type_callback:
                raise errors.DefinitionReaderError(
                    name, (f"unuspported data type definition: {type_indicator:s}")
                )
            ...
            try:
                ...
                raise errors.DefinitionReaderError(
                    definition_name, (f"in: {exception_name:s} {exception.message:s}")
                )
    ...
    def _ReadPaddingDataTypeDefinition(
        ...
        if not is_member:
            raise errors.DefinitionReaderError(
                definition_name, ("data type only supported as member")
            )
        ...
        definition_object = self._ReadDataTypeDefinition(
            ...
            definition_name,
            self._SUPPORTED_DEFINITION_VALUES_PADDING,
        )

        alignment_size = definition_values.get("alignment_size")
        if not alignment_size:
            raise errors.DefinitionReaderError(
                definition_name, ("missing alignment_size")
            )
    ...
    def _ReadStringDataTypeDefinition(
        ...
        definition_object = self._ReadElementSequenceDataTypeDefinition(
            definitions_registry,
            definition_values,
            data_types.StringDefinition,
            definition_name,
            supported_definition_values,
        )

        encoding = definition_values.get("encoding")
        if not encoding:
    ...
    def _ReadStructureFamilyDataTypeDefinition(
        ...
        base = definition_values.get("base")
        if not base:
        ...
        if not base_data_type_definition:
            raise errors.DefinitionReaderError(
                definition_name, (f"undefined base: {base:s}")
            )
        ...
        for member in members:
            ...
            if not member_data_type_definition:
                raise errors.DefinitionReaderError(
                    definition_name, (f"undefined member: {member:s}")
                )
    ...
    def _ReadStructureGroupDataTypeDefinition(
        ...
        identifier = definition_values.get("identifier")
        if not identifier:
            raise errors.DefinitionReaderError(definition_name, "missing identifier")

        default = definition_values.get("default")
        if not default:
            ...
            if not default_data_type_definition:
                raise errors.DefinitionReaderError(
                    definition_name, (f"undefined default: {default:s}")
                )
...
class DataTypeDefinitionsFileReader(DataTypeDefinitionsReader):
    ...
    def _ReadDefinition(self, definitions_registry, definition_values):
        ...
        if not type_indicator:
            raise errors.DefinitionReaderError(
                name, ("invalid definition missing type")
            )
...
class YAMLDataTypeDefinitionsFileReader(DataTypeDefinitionsFileReader):
    ...
    def _GetFormatErrorLocation(self, yaml_definition, last_definition_object):
